...
public class Flink06_Practice_ItemCountTopN {
    public static void main(String[] args) throws Exception {
        ...
        //按照商品ID分组
        SingleOutputStreamOperator<Tuple2<Long, Integer>> keyedStream = userBehaviorDS.map(new MapFunction<UserBehavior, Tuple2<Long, Integer>>() {
            @Override
        ...
        //开窗计算结果
        SingleOutputStreamOperator<ItemCount> aggregate = keyedStream.keyBy(value -> value.f0).window(SlidingEventTimeWindows.of(Time.hours(1), Time.minutes(5)))
                .aggregate(new ItemCountAggFunc(), new ItemCountWindowFunc());

        //按照窗口信息重新分组，使用状态编程的方式，实现窗口内TopN
        SingleOutputStreamOperator<String> result = aggregate.keyBy(ItemCount::getTime).process(new ItemCountProcessFunc(5));
